...
export function FileUpload({ data, getHashArray }) {
  ...
  const onDrop = useCallback(async (accFiles, rejFiles) => {
    // await accFiles.forEach(async (file) => {
    await mutateAsync(
      {
      ...
      },
      {
        onSuccess: (data, variables, context) => {
          setHashArray([...hashArray, data.fileInfoList[0]]);
        },
      }
    );
    // });
                  ...
                  return (
                    <Grid className="file-box" key={index}>
                      <Flex>
                        ...
                        <div>
                          <Grid>
                            <Text fontSize="lg" mb={1} className="file-name">
                      ...
                      <Progress className="loading" value={progress} />
                    </Grid>
                  );
                })}
